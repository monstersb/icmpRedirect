...
def main():
    ...
    opt.add_option("-v", "--victim",  action = "store", dest = "gateway", help = "victim IP address")
    opt.add_option("-t", "--target",  action = "store", dest = "target", help = "target IP you want to poisoning")
    opt.add_option("-g", "--gateway", action = "store", dest = "victim", help = "new gateway for the poisoned destination")
